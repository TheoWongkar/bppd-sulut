...
class TourPlaceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $category = BusinessCategory::where('name', 'Tempat Wisata')->first();
    ...
    public function show(string $subCategory, string $slug)
    {
        ...
        $ratingCounts = $tourPlace->reviews->groupBy('rating')->map->count();
        ...
        // Ambil data review
        $reviews = TourReview::with('user')
            ->where('tour_place_id', $tourPlace->id)
            ->latest()
            ->get();

        // Ambil data review saya
        $myReview = $tourPlace->reviews->firstWhere('user_id', Auth::id());
    ...
    public function updateReview(Request $request, string $subCategory, string $slug)
    {
        ...
        $review = TourReview::where('culinary_place_id', $tourPlace->id)
