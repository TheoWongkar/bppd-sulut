<x-main-layout>
    ...
    <header class="relative">
        ...
        <div class="absolute inset-0 bg-black/40 flex flex-col items-center justify-center text-center px-4">
            ...
            <p class="text-white text-sm mt-2 max-w-2xl text-balance">
                Beragam kuliner yang memanjakan lidah dengan sentuhan khas Sulawesi Utara
            </p>
        </div>
    </header>

    <!-- Bagian Culinary Places -->
    <section class="container mx-auto py-10 px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
                ...
                <div class="relative w-full md:w-1/4">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kuliner..."
                        autocomplete="off"
                        class="w-full border rounded-xl py-2 pl-5 focus:outline-none focus:ring-2 focus:ring-[#3b5d85] focus:border-transparent">
                    <svg class="absolute right-3 top-2.5 w-5 h-5" fill="none" stroke="currentColor" stroke-width="2"
                        viewBox="0 0 24 24">
                ...
                <aside class="w-full md:w-64">
                    <div class="bg-white rounded-lg shadow p-4">
                        ...
                        <select name="category"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#3b5d85]">
                            <option value="" {{ request('category') == '' ? 'selected' : '' }}>Semua Kategori
                            </option>
                            @foreach ($subCategories as $subCategory)
                                <option value="{{ $subCategory->slug }}"
                                    {{ request('category') == $subCategory->slug ? 'selected' : '' }}>
                                    {{ $subCategory->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @forelse ($culinaryPlaces as $culinaryPlace)
                        <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition">
                            ...
                            <div class="p-4">
                                ...
                                @php
                                    $rating = round($culinaryPlace->reviews_avg_rating ?? 0);
                                @endphp
                                ...
                                <a href="{{ route('culinaryplace.show', [$culinaryPlace->subCategory->slug, $culinaryPlace->slug]) }}"
        ...
        <!-- Pagination -->
        <div class="mt-10">
            {{ $culinaryPlaces->appends(request()->query())->links('pagination::main') }}
        </div>
    </section>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Submit form saat kategori dipilih, sambil simpan scroll
